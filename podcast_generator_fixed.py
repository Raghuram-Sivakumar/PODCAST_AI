...
CHRIS = "UEKYgullGqaF0keqT8Bu"  # Voice ID for Alex
AMELIA = "ZF6FPAbjXT4488VcRRnw"  # Voice ID for Jamie
...
def synthesize_line(text, speaker):
    ...
    try:
        ...
        print(f"[DEBUG] Synthesized {len(audio_bytes)} bytes for {speaker}")
    ...
    if not topic:
        ...
        if os.path.exists(final_audio):
            ...
            with open(final_audio, "rb") as f:
                st.download_button("📥 Download Full Podcast", f, file_name="ai_podcast.mp3")
            st.success("✅ Podcast generated with alternating voices.")
        else:
            st.error("❌ Failed to generate combined podcast.")

        # Optional: Clean up temporary segments
